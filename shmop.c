...
main () {
  ...
  int *my_ptr;		/* Coba point ke addr dengan type int. */
  ...
  while (action = ask()) {
    ...
    switch (action) {
      case 1:	/* Shmat requested. */
        ...
        if (nap == MAXnap) {
          (void) fprintf (stderr, "%s %d %s\n",
            "This simple example will only allow", MAXnap,
            "attached segments.");
          break;
        }
        p = &ap[nap++];
        ...
        if (p->shmaddr == (char *) -1) {
          ...
          (void) fprintf (stderr, "shmop: shmat returned %#8.8x %d %d\n",
            p->shmaddr, p->shmaddr, *p->shmaddr);
        }
        break;
      case 2:	/* Shmdt requested. */
        /* Get the address, make the call, report the results,
        and make the internal state match. */
        (void) fprintf (stderr, "Enter desired detach shmaddr: ");
        (void) scanf ("%i", &addr);
      ...
      case 3:	/* Read from segment requested. */
        ...
        (void) fprintf (stderr, "Enter address of an attached segment: ");
        (void) scanf ("%i", &addr);

        if (good_addr (addr))
          (void) fprintf (stderr, "String @ %#x is `%s'\n",
            addr, addr);
        (void) fprintf (stderr, "sizeof(char)=%d, sizeof(int)=%d, sizeof(addr)=%d, %d\n",
          sizeof(char), sizeof(int), sizeof(*addr), 'a');
        break;
      case 4:	/* Write to segment requested. */
        ...
        /* Set up SIGSEGV catch routine to trap attempts to write
        into a read-only attached segment. */
        ...
        if (setjmp (segvbuf)) {
          ...
          if (good_addr (addr)) {
            ...
            (*addr)++;
            write_str = atoi (addr);
            (void) sprintf (addr, "%d", ++write_str);
            write_str = atoi (addr+1);
            (void) sprintf (addr+1, "%d", ++(++write_str));
            */
            /*
            (void) strcpy (addr, msg);
            (void) fflush (stdin);
            (void) gets (addr);
            (void) scanf ("%s", addr);
            (void) fflush (stdin);
...
static ask ()
{
  ...
  do {
    (void) fprintf (stderr, "\n\tshmop: Modified version: 1996.11.28 Ag.B\n");
